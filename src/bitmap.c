...
/*   Updated: 2021/02/23 19:56:32 by osamara       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	init_bmp(t_bmp *bmp, t_game_engine_state *state)
{
	...
	bmp->bmpheader.filesize = pixel_byte_size + sizeof(t_bmp);
	bmp->bmpheader.reserved_bytes = 0;
	bmp->bmpheader.pixel_data_offset = sizeof(t_bmp);
...
int	create_bmp_file(t_game_engine_state	*state)
{
	...
	while (y > 0)
	{
		write(fd, ((char*)state->window->addr + (y - 1) * state->window->line_length), img_width);
		y--;
	}
	return (SUCCESS);
}
