...
/*   Updated: 2021/02/04 21:15:34 by osamara       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
double		calc_dist_on_horiz_inters(t_engine_state *engine_state, t_map *map)
{
	...
	horiz_intersection.x = engine_state->pos_x + fabs((engine_state->pos_y - horiz_intersection.y) * tan(engine_state->radian));
	horiz_intersection.step_x = TILE_SIZE * fabs(tan(engine_state->radian));
	...
	return (distance_to_wall(&horiz_intersection, engine_state, map));
}
...
double		calc_dist_on_vert_inters(t_engine_state *engine_state, t_map *map)
{
	...
	vert_intersection.y = engine_state->pos_y - fabs((engine_state->pos_x - vert_intersection.x) / tan(engine_state->radian));
	vert_intersection.step_y = -TILE_SIZE / fabs(tan(engine_state->radian));
	if (engine_state->ray_angle >= 90 && engine_state->ray_angle < 270)
	{
		vert_intersection.y = engine_state->pos_y + fabs((engine_state->pos_x - vert_intersection.x) / tan(engine_state->radian));
	...
	printf("vert_x:%g\n", vert_intersection.x);
	printf("vert_y:%g\n", vert_intersection.y);
	return (distance_to_wall(&vert_intersection, engine_state, map));
}
